Create one horizontal animation strip for Codex pet `do-hwan`, state `running-right`.
...
Identity: same pet in every frame: Compact cute baby mascot based on the reference photo: round cheeks, cream knit cap with top knot, green-and-cream striped romper, white socks, friendly curious expression. Use a soft sticker or plush-like pet-safe style, not photorealistic.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `sticker`: Polished sticker mascot with bold clean shapes, crisp outline, flat colors, and minimal highlight detail. User style notes: Soft rounded sticker mascot with clean outlines, readable at small size, no text, no logos, no scenery..
